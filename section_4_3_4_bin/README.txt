...
1. The zip file you downloaded contains this README file and the binary code for the example in Section 4.3.4. of the book.

2. Unzip the contents of the zip file into a local folder on your machine,let us call it the 'app' folder. The folder would contain the following files :
...
3. Browse to the lib folder inside the 'db4o' folder.Typically it would be something like db4o\db4o-8.0-java\db4o-8.0\lib.

4. You will need to create a folder let us call it 'dep' inside the 'app' folder. 

5. Copy a file similar to 'db4o-8.0.249.16098-all-java5.jar' from 'db4o\db4o-8.0-java\db4o-8.0\lib' to the 'app\dep' folder.
...
2. From the command prompt, navigate to the folder where contents of the downloaded zip file were extracted to. 

3. Type the command 'java -cp .;./*;./dep/* codabook.componentrunner.ComponentRunner posapp.app.POSAPP' to start execution of the program which would present a text based menu.

4. Use the 'Point-Of-Sale' program as guided by the text based menu titled 'Welcome to Resturant'. 
